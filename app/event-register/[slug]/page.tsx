...
import invitee from "@/services/invitee";
import {Invitation} from "@/@types/invitation";
import {DownloadWallet} from '../../ui/DownloadWallet';

export default function EventRegister({params}: { params: { slug: string } }): JSX.Element {
  const [inviteeData, setInviteeData] = useState<Invitation | null>(null)
  const [isLoading, setIsLoading] = useState<boolean>(false)

  useEffect(() => {
    setIsLoading(true)
    invitee.getInvitee(params.slug).then(resp => {
      setInviteeData(resp)
      setIsLoading(false)
    }).catch(err => {
      console.error(err);
    ...
    <main className="mx-auto w-full max-w-screen-lg p-8">
      {!isLoading ? <div>
          ...
          <p className="pb-8 text-center text-lg">
            {inviteeData?.event?.location}
          </p>
          <div className="mt-8 flex flex-col gap-8 xl:flex-row">
            <DownloadWallet/>
            <div className="flex flex-col items-center">
              {inviteeData?.event && <QRCode size={200} value={JSON.stringify(inviteeData)} className="w-full"/>}
              <p className="mt-4">
                <Link
                  href="/credentials"
